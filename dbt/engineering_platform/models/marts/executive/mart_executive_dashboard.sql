...
cost_summary AS (
    ...
    FROM {{ ref('stg_cost') }} fc
    INNER JOIN {{ ref('stg_project') }} dp ON fc.project_key = dp.project_key
),
...
schedule_summary AS (
    ...
    FROM {{ ref('stg_schedule') }} fs
    INNER JOIN {{ ref('stg_project') }} dp ON fs.project_key = dp.project_key
),
