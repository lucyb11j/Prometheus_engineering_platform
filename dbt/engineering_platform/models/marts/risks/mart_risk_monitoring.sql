...
WITH risk_detail AS (
    ...
    INNER JOIN {{ ref('stg_date') }} dd ON fr.date_key = dd.date_key
    INNER JOIN {{ ref('stg_project') }} dp ON fr.project_key = dp.project_key
    INNER JOIN {{ ref('stg_risk') }} dr ON fr.risk_key = dr.risk_key
),
