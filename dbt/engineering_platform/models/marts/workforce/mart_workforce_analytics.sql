...
WITH workforce_detail AS (
    ...
    INNER JOIN {{ ref('stg_date') }} dd ON fw.date_key = dd.date_key
    INNER JOIN {{ ref('stg_employee') }} de ON fw.employee_key = de.employee_key
    INNER JOIN {{ ref('stg_project') }} dp ON fw.project_key = dp.project_key
),
